...
newtool_collect::newtool_collect() {
...
void newtool_collect::run() {

	fstream f;
	f.open(".\\dataset\\dataset_AE_fresh.txt", ios::app);//dataset_AE_breakage     dataset_AE_fresh
	if (!f.is_open()) {
		cout << "dataset_AE_fresh file open!" << endl;
		return;
	}
	float Volt = 0;
	LONG ADData;
